...
# ==============================================================================
# Description: Production environment EKS infrastructure (based on working dev/test)
...
module "networking" {
  ...
  single_nat_gateway       = false  # Multi-AZ for production HA
...
module "kms" {
  ...
  project_name = var.project_name
  environment  = local.environment
  create_ebs_kms_key = true  # Enable for production security
...
module "eks_cluster" {
  ...
  cluster_endpoint_public_access  = false  # Disabled for maximum production security
