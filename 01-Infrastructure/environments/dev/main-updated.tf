...
module "kms" {
  ...
  # Key Configuration (Minimal for dev cost optimization)
  create_eks_key              = true
  create_ebs_key             = false  # Use default EBS encryption in dev
  create_cloudwatch_logs_key = false  # Use default encryption in dev
  create_secrets_manager_key = false  # Use default encryption in dev
  create_s3_key              = false  # Use default S3 encryption in dev
  create_dynamodb_key        = false  # Use default encryption in dev

  # Gaming-specific encryption requirements (minimal for dev)
  gaming_encryption_requirements = {
    encrypt_session_data    = false  # Disabled for dev performance
    encrypt_player_data     = false  # Disabled for dev performance
    encrypt_game_assets     = false
    encrypt_telemetry       = false
    encrypt_backups         = false
    high_performance_mode   = true   # Enabled for dev testing
  }
...
module "iam" {
  ...
  # Gaming-specific features (enabled for dev testing)
  gaming_features = {
  ...
  # Environment-specific configurations (dev-friendly)
  environment_config = {
    enable_debug_policies  = true   # Allow debugging in dev
    enable_admin_access    = true   # Full access for dev
    restrict_to_region     = true
    enable_cost_monitoring = false  # Disabled for dev
  }
